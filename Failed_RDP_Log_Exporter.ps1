...
$API_KEY      = "c926304cb4784d288d539a315b6089d7"
...
<#
    This function creates a bunch of sample log files that will be used to train the Extract feature in Log Analytics  workspace.
    We can avoid including these fake records on our map by filtering out all logs witha destination host of "samplehost".
#>
...
while ($true)
{
    ...
    foreach ($event in $events) {
        ...
        if ($event.properties[19].Value.Length -ge 5) {
            ...
            $eventId = $event.Id
            $destinationHost = $event.MachineName # Destination Name
            $username = $event.properties[5].Value # Account Name (Attempted Logon)
            $sourceHost = $event.properties[11].Value # Source Name
